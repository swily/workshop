...
if [ -f "../cleanup/clean_cluster.sh" ]; then
  echo "Using clean_cluster.sh from cleanup directory"
  ../cleanup/clean_cluster.sh
else
  echo "Warning: clean_cluster.sh not found in cleanup directory, skipping cleanup"
fi
...
# Use the observability version of configure_otel_demo.sh that skips Prometheus/Grafana
if [ -f "../configure_otel_demo_observability.sh" ]; then
  # Use the observability version that skips Prometheus/Grafana installation
  ../configure_otel_demo_observability.sh
else
  echo "Error: configure_otel_demo_observability.sh not found"
  echo "This script is required for New Relic integration"
  exit 1
fi
