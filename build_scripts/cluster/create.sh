...
while getopts "n:" opt; do
  case $opt in
    ...
    \?)
      ...
      echo "Usage: $0 [-n cluster_name]" >&2
      exit 1
      ;;
  esac
done

# Use default name if CLUSTER_NAME is not set via environment variable or command line
if [ -z "${CLUSTER_NAME}" ]; then
  export CLUSTER_NAME=current-workshop
  echo "No cluster name provided. Using default: ${CLUSTER_NAME}"
fi

export OWNER=$(whoami)
export EXPIRATION=$(date -v +7d +%Y-%m-%d)

# Display the cluster name that will be created
echo "Creating cluster with name: ${CLUSTER_NAME}"

# Check if cluster already exists
if aws eks describe-cluster --name ${CLUSTER_NAME} --region ${AWS_REGION} &>/dev/null; then
  echo "Error: Cluster '${CLUSTER_NAME}' already exists. Please choose a different name or delete the existing cluster."
  exit 1
fi
