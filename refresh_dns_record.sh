...
echo "Updating DNS record for ${CLUSTER_NAME}.gremlinpoc.com to point to ${canonical_hosted_zone_name}"
aws route53 change-resource-record-sets --hosted-zone-id "$r53_zone_id" --change-batch \
"{ \"Changes\": [ { \"Action\": \"UPSERT\", \"ResourceRecordSet\": { \"Name\": \"${CLUSTER_NAME}.gremlinpoc.com\", \"Type\": \"A\",\"AliasTarget\": { \"HostedZoneId\": \"$canonical_hosted_zone_id\", \"DNSName\": \"$canonical_hosted_zone_name\", \"EvaluateTargetHealth\":true } } } ] }" | jq .
